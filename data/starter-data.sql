INSERT INTO blog_author(blog_author_id, first_name, last_name) VALUE (1, 'Matthew', 'Maxwell');

INSERT INTO blog_post_status(blog_post_status_id, status) VALUE (1, 'DRAFT');
INSERT INTO blog_post_status(blog_post_status_id, status) VALUE (2, 'PUBLISHED');
...
VALUE (1,
       ...
       'https://mattmaxwell/images/rotor.png');
...
VALUE (2,
       ...
       'https://mattmaxwell/images/operation-home.png');
...
VALUE (3,
       ...
       'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Rhoncus urna neque viverra justo nec ultrices. Nunc faucibus a pellentesque sit amet porttitor eget dolor morbi. Libero enim sed faucibus turpis in eu mi bibendum neque. Id aliquet risus feugiat in. Tortor at auctor urna nunc id. Eu turpis egestas pretium aenean. Suspendisse sed nisi lacus sed viverra tellus in. Quis commodo odio aenean sed. Non arcu risus quis varius quam quisque id diam vel.<br>```router.put(\'/posts/:uri\', passport.authenticate(\'jwt\', { session: false }), (req, res, next) => {    const postData = {        _id:            req.body._id || new mongoose.Types.ObjectId(),        uri:            req.body.uri,        title:          req.body.title,         subtitle:       req.body.subtitle,        topics:         req.body.topics,        author:         req.body.author,        description:    req.body.description,        content:        req.body.content,        imageURL:       req.body.imageURL,        updated:        Date.now()    };    Profile.updateMany({}, {$addToSet: {posts: postData._id}}, (err, result) => {        if(err) throw err;    });    Topic.updateMany({}, {$pull: {posts: postData._id}}, (err, result) => {        if(err) throw err;    });    Topic.updateMany({_id: {$in: postData.topics}}, {$push: {posts: postData._id}}, (err, result) => {        if(err) throw err;    });    Post.updateOne({_id: postData._id}, postData, (err, result) => {        if(err) throw err;        if(result.nModified === 0) {            const newPost = new Post({                ...postData,                created: Date.now()            });            newPost.save((err, post) => {                if(err) {                    res.sendStatus(400);                } else {                        res.sendStatus(201);                }            });        } else {            res.sendStatus(200);        }    });});```<br>Sed adipiscing diam donec adipiscing tristique. Interdum velit euismod in pellentesque massa. Ultricies lacus sed turpis tincidunt id aliquet risus feugiat in. Posuere urna nec tincidunt praesent semper feugiat nibh sed pulvinar. Facilisi cras fermentum odio eu feugiat pretium nibh ipsum. Mattis molestie a iaculis at. Amet porttitor eget dolor morbi non. Elit scelerisque mauris pellentesque pulvinar pellentesque habitant morbi tristique senectus. Ut morbi tincidunt augue interdum velit. Vel quam elementum pulvinar etiam non quam. Vulputate odio ut enim blandit volutpat maecenas volutpat. Sollicitudin nibh sit amet commodo nulla facilisi nullam vehicula. Convallis posuere morbi leo urna molestie at elementum eu. Condimentum mattis pellentesque id nibh tortor id aliquet lectus. Amet venenatis urna cursus eget nunc scelerisque viverra mauris.',
       'https://mattmaxwell/images/green-foot.png');

INSERT INTO blog_topic(blog_topic_id, name, description) VALUE (1, 'Audio Development', 'Any and all things related to audio development and audio programming.');
INSERT INTO blog_topic(blog_topic_id, name, description) VALUE (2, 'Design', 'Any and all things related to software design.');
INSERT INTO blog_topic(blog_topic_id, name, description) VALUE (3, 'Engineering', 'Any and all things related to software engineering.');
INSERT INTO blog_topic(blog_topic_id, name, description) VALUE (4, 'Game Development', 'Any and all things related to game development.');
INSERT INTO blog_topic(blog_topic_id, name, description) VALUE (5, 'Mathematics', 'Any and all things related to mathematics (most likely DSP-related concepts).');
INSERT INTO blog_topic(blog_topic_id, name, description) VALUE (6, 'Programming', 'Any and all things related to programming.');

INSERT INTO blog_post_topic_mapping(blog_post_id, blog_topic_id) VALUE (1, 1);
INSERT INTO blog_post_topic_mapping(blog_post_id, blog_topic_id) VALUE (2, 4);
INSERT INTO blog_post_topic_mapping(blog_post_id, blog_topic_id) VALUE (3, 3);

INSERT INTO profile_status(profile_status_id, status) VALUE (1, 'ACTIVE');
INSERT INTO profile_status(profile_status_id, status) VALUE (2, 'INACTIVE');

INSERT INTO profile(profile_id, profile_status_id, name, tagline, landing, about)
VALUE (1,
       2,
       'Audio Development',
...
INSERT INTO project(project_id, name, tagline, description, image_url, external_url)
VALUE (1,
       'Counterplay',
...
INSERT INTO technology(technology_id, profile_id, name, icon_url, display_order) VALUE (1, 1, 'C++', 'url-to-icon', 1);
INSERT INTO technology(technology_id, profile_id, name, icon_url, display_order) VALUE (2, 1, 'JUCE', 'url-to-icon', 2);
INSERT INTO technology(technology_id, profile_id, name, icon_url, display_order) VALUE (3, 2, 'C#', 'url-to-icon', 1);
INSERT INTO technology(technology_id, profile_id, name, icon_url, display_order) VALUE (4, 2, 'Unity', 'url-to-icon', 2);
INSERT INTO technology(technology_id, profile_id, name, icon_url, display_order) VALUE (5, 3, 'TypeScript', 'url-to-icon', 1);
INSERT INTO technology(technology_id, profile_id, name, icon_url, display_order) VALUE (6, 3, 'Angular', 'url-to-icon', 2);
INSERT INTO technology(technology_id, profile_id, name, icon_url, display_order) VALUE (7, 3, 'JavaScript (ES6+)', 'url-to-icon', 3);
INSERT INTO technology(technology_id, profile_id, name, icon_url, display_order) VALUE (8, 3, 'React & Redux', 'url-to-icon', 4);
INSERT INTO technology(technology_id, profile_id, name, icon_url, display_order) VALUE (9, 3, 'Scala', 'url-to-icon', 5);
INSERT INTO technology(technology_id, profile_id, name, icon_url, display_order) VALUE (10, 3, 'Play', 'url-to-icon', 6);
INSERT INTO technology(technology_id, profile_id, name, icon_url, display_order) VALUE (11, 3, 'Express', 'url-to-icon', 7);
INSERT INTO technology(technology_id, profile_id, name, icon_url, display_order) VALUE (12, 3, 'Node', 'url-to-icon', 8);
INSERT INTO technology(technology_id, profile_id, name, icon_url, display_order) VALUE (13, 3, 'MySQL', 'url-to-icon', 9);
INSERT INTO technology(technology_id, profile_id, name, icon_url, display_order) VALUE (14, 3, 'MongoDB', 'url-to-icon', 10);