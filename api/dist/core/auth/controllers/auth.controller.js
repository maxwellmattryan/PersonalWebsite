...
let AuthController = class AuthController {
    ...
    async register(adminData) {
        throw new auth_exception_1.NotAllowedToRegisterException();
    }
    async login(adminData, response) {
        const admin = await this.authService.authenticateAdmin(adminData);
        if (!admin)
            throw new auth_exception_1.WrongCredentialsWereProvidedException();
